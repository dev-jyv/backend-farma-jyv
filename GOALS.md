...
- **Medicamentos controlados (COFEPRIS)** — grupos I–VI en el producto, reglas de receta/folio/retención por grupo, cédula profesional validada y libro de control auditable con contra-asientos en anulaciones y devoluciones.
...
- **Punto de venta** — ventas transaccionales que descuentan stock vía FEFO, registran método de pago (`cash`/`card`/`transfer`/`mixed`), folio, cajero, cliente opcional y receta cuando el producto la exige. Cancelación de venta reservada al rol `admin`.
...
- Deuda de datos: documentos históricos sin `totalStock` / `productIds` / `batches.supplierId`; requiere backfill (ver P1 del roadmap).
...
### P0 — el POS no es seguro de operar sin esto

| Módulo | Qué falta hoy |
|---|---|
| ~~**Idempotencia de venta**~~ | **Hecho (2026-08-03).** `createSale` acepta `idempotencyKey`; falta que el frontend la envíe siempre y volverla obligatoria en el schema. |
| ~~**Desglose de impuestos**~~ | **Hecho (2026-08-03).** Precios con impuestos incluidos; desglose por partida en `SaleItem.taxes` y total en `Sale.taxSummary`. Ventas históricas quedan con `taxSummary: null` (sin backfill). |
| ~~**Devoluciones parciales**~~ | **Hecho (2026-08-03).** `POST /v1/sale-returns` con reingreso al lote de origen, movimiento `return_in` y reembolso Point automático. |
| ~~**Ticket / comprobante**~~ | **Hecho (2026-08-03).** `GET /v1/sales/:id/receipt` y `/v1/sale-returns/:id/receipt` devuelven JSON + HTML para rollo térmico 58/80mm. |

### P1 — obligatorio para farmacia formal en México

| Módulo | Qué falta hoy |
|---|---|
| **Facturación CFDI 4.0** | Hay snapshot `billing` + `invoiceStatus: 'pending'`, pero nada timbra. Falta PAC (Facturama/SW/Finkok), factura global de público en general, cancelación con motivo, entrega de PDF+XML. |
| ~~**Caducidades y stock bajo**~~ | **Hecho (2026-08-03).** `GET /v1/inventory/alerts` + correo diario `dailyInventoryAlerts` (07:00 MX) que se calla cuando no hay novedades. |
| ~~**Ajuste por conteo físico**~~ | **Hecho (2026-08-03).** `POST /v1/inventory/counts` con movimiento `adjustment_count` con signo y acta en `inventoryCounts`. |
| ~~**Medicamentos controlados**~~ | **Hecho (2026-08-03).** Grupos I–VI, reglas de receta/folio/retención, cédula validada y libro de control `controlledSalesLedger` con contra-asientos. |
| ~~**Bitácora de acciones**~~ | **Hecho (2026-08-03).** `auditLogs` + `GET /v1/audit-logs` sobre dinero, precios y accesos. |
| ~~**Backfill de campos denormalizados**~~ | **Script listo (2026-08-03):** `npm run backfill:denormalized` (idempotente, `--dry-run`, `--force`, `--only=`). Falta **correrlo en producción** y luego quitar los fallbacks de `totalStock`. |

Nota sobre el backfill (ya implementado con este patrón): seguir el patrón de [functions/src/scripts/migrate-roles.ts](functions/src/scripts/migrate-roles.ts) — script en `scripts/`, compilado a `lib/scripts/`, disparado por su propio `npm run` (como `migrate:roles`); no inventar otro mecanismo. Es prerequisito de "Reportes adicionales" (top vendidos / sin movimiento se apoyan en `productIds`) y de "Devolución a proveedor / canje de caducados" (`batches.supplierId`).

### P1.5 — deuda descubierta al implementar lo anterior

| Tema | Qué pasa |
|---|---|
| **Semántica de pago `mixed`** | `resolveTender` exige que el efectivo recibido cubra el **total completo** incluso en pago mixto, donde por definición una parte la paga la tarjeta. Hoy obliga al cajero a capturar efectivo de más y el cambio sale mal. Corregir junto con el cálculo de `cashInDrawer`. |
| **CFDI ya desbloqueado** | Con el desglose por partida y el `usoCfdi` del cliente, el módulo de timbrado (P1) ya tiene todos los datos que necesita; falta solo el PAC. |
...
| ~~**Corte X (lectura parcial)**~~ | **Hecho (2026-08-03).** `GET :id/x-report` (vista previa) y `POST :id/x-report` (lectura registrada, folio `X-`), ambos imprimibles. |
| ~~**Reportes adicionales**~~ | **Hecho (2026-08-03).** `/v1/reports/{sales-summary,profit,top-products,by-cashier,dead-stock}` con margen sobre base sin impuestos. |
| ~~**GS1-128 / DataMatrix**~~ | **Hecho (2026-08-03).** `POST /v1/inventory/scan` resuelve GTIN, lote y caducidad del código impreso. |
