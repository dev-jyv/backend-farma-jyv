...
- `npm run test:coverage` aplica `coverageThreshold` de `jest.config.js`. El piso global va ~2 puntos por debajo de lo medido a propósito: las suites comparten emulador y reutilizan lo que dejaron las anteriores, así que la cifra oscila ~1 punto entre corridas y un umbral al filo falla de forma intermitente. Gates estrictos donde importa: `auth.guard.ts` y `memory-cache.ts` al 100% de líneas, `users.service.ts` al 80%. `src/scripts/**` excluido.
- Flake conocido: `sales.spec.ts › dos requests concurrentes con la misma llave produce una sola venta` falla ~1 de cada 4 corridas con la suite completa y pasa aislado. Es contención del emulador, no la lógica de idempotencia.
